...
include_once($phpbb_root_path . 'konkurs/konkursrank.inc.' . $phpEx);
...
$mod_action = append_sid("{$phpbb_root_path}mcp.$phpEx", "f=$forum_id&amp;t=$topic_id&amp;quickmod=1&amp;redirect=" . urlencode(str_replace('&amp;', '&', $viewtopic_url)), true, $user->session_id);
...
$template->assign_vars(array(
	...
	'U_MCP_FORUM'	=> ($auth->acl_get('m_', $forum_id)) ? append_sid("{$phpbb_root_path}mcp.$phpEx", "i=main&amp;mode=forum_view&amp;f=$forum_id", true, $user->session_id) : '',
	'U_MCP_TOPIC'	=> ($auth->acl_get('m_', $forum_id)) ? append_sid("{$phpbb_root_path}mcp.$phpEx", "i=main&amp;mode=topic_view&amp;f=$forum_id&amp;t=$topic_id" . (($start == 0) ? '' : "&amp;start=$start") . ((strlen($u_sort_param)) ? "&amp;$u_sort_param" : ''), true, $user->session_id) : '',
	...
	'U_EMAIL_TOPIC'			=> ($user->data['is_registered'] && $auth->acl_get('f_email', $forum_id) && $config['email_enable']) ? append_sid("{$phpbb_root_path}memberlist.$phpEx", "mode=email&amp;t=$topic_id") : '',
	...
	if (!isset($user_cache[$poster_id]))
	{
		...
		else
		{
			...
			$user_cache[$poster_id] = array(
				...
				'rank_image_konkurs'	=> $kRank->getRank($poster_id),
//				'rank_image_konkurs'	=> '',
...
for ($i = 0, $end = sizeof($post_list); $i < $end; ++$i)
{
	...
	$postrow = array(
		...
		'RANK_IMG_KONKURS'	=> $user_cache[$poster_id]['rank_image_konkurs'],
...
$gallery_block->set_display_options(array());
$gallery_block->set_nums(array('rows' => 1, 'columns' => 6, 'comments' => 0, 'contests' => 0));
$gallery_block->add_albums(array(1));
$gallery_block->set_pegas(true);
$gallery_block->set_template_block_name('imageblocktop');
